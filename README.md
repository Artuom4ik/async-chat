...
- Если вы в первый раз запускаете программу и у вас нету токена для авторизации, то программа предложит зарегестрироваться
...
- То добавьте этот в папку `graphical_app` и переименуйте как `auth.json`

- После успешной регистрации, откроется графический интерфес программы, где отобразиться история сообщений и статус соединения с сервером для чтения и отправки, а также ваш `nickname`, под которым вы зарегестрировались.
